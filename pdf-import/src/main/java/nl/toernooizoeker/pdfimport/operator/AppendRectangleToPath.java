package nl.toernooizoeker.pdfimport.operator;

import nl.toernooizoeker.pdfimport.Rectangle;
import nl.toernooizoeker.pdfimport.TableCellIterator;
import org.apache.pdfbox.cos.COSBase;
import org.apache.pdfbox.cos.COSNumber;

import java.util.List;

public class AppendRectangleToPath extends OperatorProcessor {
    public AppendRectangleToPath(TableCellIterator processor) {
        super(processor);
    }

    @Override
    public void process(List<COSBase> operands) {
        COSNumber x = (COSNumber) operands.get(0);
        COSNumber y = (COSNumber) operands.get(1);
        COSNumber width = (COSNumber) operands.get(2);
        COSNumber height = (COSNumber) operands.get(3);
        processor.appendRectangleToPath(Rectangle.of(x.intValue(), y.intValue(), width.intValue(), height.intValue()));
    }
}
